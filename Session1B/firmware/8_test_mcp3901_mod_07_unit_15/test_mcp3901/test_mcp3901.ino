...
void loop() {
  ...
  byte dataRead[3];  // max of 3 bytes are ever returned
  ...
  bytes[0] = configA | 0b10000000; // set the PRESCALE to 0b10 which is for a 16MHZ clock and that's the crystal I used.  We need to clock down to 4MHZ and 0b10 means Clock/4
  configA = configA | 0b00001000;   // set the number of bits per measure to 0b1 which means 24 bit values
  bytes[1] = configB | 0b00000010;  // turning on VREFEXT
  ...
  memset(dataRead,0,sizeof(dataRead));
  mcp3901ReadRegister(0x00, 3, dataRead);  // this will read back A and B
  long adcCh1Value = 0;
  adcCh1Value = (dataRead[0] << 16) | (dataRead[1] << 8 ) | dataRead[2];
  Serial.println("Value from channel 1");
  Serial.print("   dec: ");
  Serial.println(adcCh1Value);
  Serial.print("   bin: ");
  print_binary(adcCh1Value, 24);  
  Serial.println("");
  // reference voltage is 2.37V
  long maxAdcValue = 0xFFF; // 24 bits
  float voltageRef = 2.37;
  float voltsMeasured = (adcCh1Value/maxAdcValue) * voltageRef;
  Serial.print(" volts: ");
